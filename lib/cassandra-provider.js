...
p.get = function(key, cb) {
    var info = url.parse(key, false);
	var args = arguments;
	execCQL.call(this, "SELECT cache FROM httpCache WHERE host = ? AND path = ?",
		[info.host || "null", info.path],
...
p.set = function(key, cache, ttl, cb) {
	...
	execCQL.call(this,
		"INSERT INTO httpCache (host, path, cache, expires) VALUES (?, ?, ?, ?) USING TTL ?",
		[info.host || "null", info.path, val, (Date.now() + (ttl * 1000)), ttl],
...
p.remove = function(key, cb) {
    var info = url.parse(key, false);
	var args = arguments;
	execCQL.call(this, "DELETE FROM httpCache WHERE host = ? AND path = ?",
		[info.host || "null", info.path],
		{ consistency: helenus.ConsistencyLevel.ALL },
		function(err) {
			cb && cb(err);
...
function execCQL(cql, data, options, cb) {
	...
	if (!pool) {
		...
		pool.connect(function (err, keyspace) {
            if (err) {
                pool.errors = 1;
                setTimeout(function() { // all existing requests will fail, but will permit reconnect after timeout
                    delete $this.pools[consistency];
                }, 15000);
            } else {
    		    pool.isReady = true;
			    var i;
